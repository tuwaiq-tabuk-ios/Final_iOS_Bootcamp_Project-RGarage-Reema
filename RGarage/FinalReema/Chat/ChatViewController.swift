...
//  }
  ...
  func loadData(){
      db.collection("messages").order(by: "time", descending: true)
            .whereField("users",arrayContains: (Auth.auth().currentUser?.uid ?? "") as String).addSnapshotListener { (querySnapshot, erorr) in
        if let snapshotDoc = querySnapshot?.documents{
          self.messages = []
          for doc in snapshotDoc{
            let data = doc.data()
